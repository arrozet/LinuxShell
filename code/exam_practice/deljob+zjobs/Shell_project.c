...
void manejador_child(int signal){	// para que no se queden zombies los hijos
	...
	for(int i=2; task!=NULL; i++){	// He destrozado el manejador para que se queden zombies 
		//debug(pid_wait, %u);

		pid_wait = waitpid(task->pgid, &status, WNOHANG | WUNTRACED | WCONTINUED);
		if(pid_wait==task->pgid){	// Si el que acaba de terminar es la tarea que busco
			status_res = analyze_status(status, &info);
			if(task!=NULL){
		...
		task=task->next;
		task = get_item_bypos(lista_jobs, i);
	}
	unblock_SIGCHLD();
}
...
void traverse_proc(pid_t pid_shell) {
    DIR *d; 
    struct dirent *dir;
    char buff[2048];
    d = opendir("/proc");
...
int main(void)
{
	...
	{   		
		...
		//BG
			...
			block_SIGCHLD();
			job* curr = get_item_bypos(lista_jobs, 1);
			if(curr!=NULL && curr->state!=STOPPED){
				printf("Borrando trabajo actual de la lista de jobs: PID=%u command=%s\n", curr->pgid, curr->command);
				delete_job(lista_jobs, curr);
			}
			else if(curr!=NULL && curr->state==STOPPED){
			...
			printf("\n");	// SI NO PONES ESTO DA ERROR (JAJA GRACIOSO ERROR)
			traverse_proc(pid_shell);
			continue;
		}
		/* the steps are:
